#!/usr/bin/env python3
"""
Script to update the React app with the current LangGraph workflow.
...
def generate_react_workflow(nodes, edges):
    ...
    for i, node in enumerate(other_nodes):
        if node not in positions:
            positions[node] = {'x': (i % 5) * col_width, 'y': y_offset + (i // 5) * row_height}
...
def update_react_app(nodes, edges):
    ...
    # Replace the initialNodes definition
    # Find the initialNodes variable and replace its value
    pattern_nodes = r'(const initialNodes = )\[([^\]]*?)\];'
    replacement_nodes = f'const initialNodes = {nodes_js};'
    content = re.sub(pattern_nodes, replacement_nodes, content, count=1, flags=re.DOTALL)
    
    # Replace the initialEdges definition
    # Find the initialEdges variable and replace its value  
    pattern_edges = r'(const initialEdges = )\[([^\]]*?)\];'
    replacement_edges = f'const initialEdges = {edges_js};'
    content = re.sub(pattern_edges, replacement_edges, content, count=1, flags=re.DOTALL)
    ...
    print("RAG nodes included:", [node for node in nodes if 'rag' in node.lower() or 'document' in node.lower() or 'context' in node.lower()])
