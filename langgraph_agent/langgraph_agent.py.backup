...
class AgentState(TypedDict):
    ...
    rag_context: str  # Augmented context with retrieved documents
    use_rag_flag: bool  # Whether to use RAG for this request
    rag_relevance_score: float  # Relevance score of retrieved documents
    rag_query: str  # The query used for document retrieval
    rag_response: str  # Response generated using RAG
...
def create_enhanced_agent_graph():
    ...
    # Create a function to check if RAG should be used instead of traditional SQL approach
    def should_use_rag(state: AgentState) -> Literal["use_rag", "use_sql"]:
        """
        Conditional edge to determine if we should use RAG or traditional SQL approach
        """
        ...
        if disable_databases:
            ...
            return "use_sql"  # Proceed with existing MCP flow

        if use_rag:
            logger.info("Using RAG approach for the request")
            return "use_rag"
        else:
            logger.info("Using traditional SQL approach for the request")
            return "use_sql"
    ...
    workflow.add_conditional_edges(
        ...
        should_skip_database_operations,
        {
            ...
            "continue_with_dbs": "check_rag_applicability"   # Continue with RAG check
        }
    )

    # Add conditional edge for RAG vs SQL decision
    workflow.add_conditional_edges(
        "check_rag_applicability",
        should_use_rag,
        {
            "use_rag": "retrieve_documents",  # Use RAG approach
            "use_sql": "discover_services"   # Use traditional SQL approach
        }
    )
...
def check_rag_applicability_node(state: AgentState) -> AgentState:
    ...
    Determines whether to use RAG or proceed with traditional SQL approach.
    """
    start_time = time.time()
    user_request = state["user_request"]
    ...
    try:
        ...
        # Determine if the request is better suited for RAG than SQL
...
def augment_context_node(state: AgentState) -> AgentState:
    ...
    """
    start_time = time.time()
    logger.info(f"[NODE START] augment_context_node - Augmenting context with retrieved documents")
    ...
    try:
        ...
        context_parts = ["Use the following context to answer the question:"]
        ...
        elapsed_time = time.time() - start_time
        logger.info(f"[NODE SUCCESS] augment_context_node - Augmented context with {len(rag_docs)} documents in {elapsed_time:.2f}s")
        ...
        # Return original user request as context on error
        return {
            **state,
    ...
    try:
        ...
        return {
            **state,
            ...
            "final_response": rag_response  # Set as final response since we're using RAG
        }
    except Exception as e:
        elapsed_time = time.time() - start_time
