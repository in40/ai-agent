...
def create_enhanced_agent_graph():
    ...
    def should_use_rag(state: AgentState) -> Literal["use_rag", "use_mcp"]:
        ...
        """
        use_rag = state.get("use_rag_flag", False)
        disable_databases = state.get("disable_databases", False)
        ...
        if use_rag:
            logger.info("Using RAG approach for the request")
            return "use_rag"
        else:
    ...
    workflow.add_edge("retrieve_documents", "augment_context")
    workflow.add_edge("augment_context", "generate_rag_response")
    workflow.add_edge("generate_rag_response", END)  # RAG responses end the workflow
...
# Removed old MCP-related nodes that are no longer used in the new workflow


# Removed old RAG-related nodes that are no longer used in the new workflow
