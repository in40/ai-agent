...
const RAGComponent = () => {
  ...
  const toggleExpand = (index) => {
    if (expandedDocIndex === index) {
      setExpandedDocIndex(null); // Collapse if already expanded
    } else {
      setExpandedDocIndex(index); // Expand the clicked document
    }
  };
            ...
            {results.documents && results.documents.length > 0 ? (
              <ul>
                {results.documents.map((doc, index) => (
                  <li key={index} style={{ marginBottom: '20px', border: '1px solid #ddd', borderRadius: '5px', padding: '10px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      >
                        {expandedDocIndex === index ? 'Collapse' : 'Expand'}
                      </button>
                    </div>

                    <div>
                      <p><strong>Source:</strong> {doc.metadata?.source || 'Unknown'}</p>
                      <p><strong>Score:</strong> {doc.score ? doc.score.toFixed(4) : 'N/A'}</p>
                      ...
                      {/* Show content based on expansion state */}
                      <div style={{
                        marginTop: '10px',
                        maxHeight: expandedDocIndex === index ? 'none' : '200px',
                        overflowY: expandedDocIndex === index ? 'visible' : 'auto',
                        position: 'relative'
                      }}>
                        <p style={{
                          whiteSpace: 'pre-wrap',
                        ...
                        {expandedDocIndex !== index && doc.page_content.length > 500 && (
                          <div style={{
                            position: 'absolute',
                            bottom: 0,
                            left: 0,
