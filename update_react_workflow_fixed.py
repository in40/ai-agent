...
def generate_react_workflow(nodes, edges):
    ...
    for node_id in nodes:
        ...
        node_data = {
            ...
            'data': {
                ...
                'nodeFunction': f'{node_id}_node',  # Assuming function name pattern
    ...
    for edge in edges:
        ...
        # Add conditional labels if the edge has data
        if hasattr(edge, 'data') and edge.data:
            edge_data['label'] = str(edge.data)
...
def update_react_app(nodes, edges):
    ...
    # First, let's find the initialNodes definition and replace it
    pattern_nodes = r'(const initialNodes = )\[([^\[]*?)(?=\n\s*const|$);'
    content = re.sub(pattern_nodes, f'const initialNodes = {nodes_js};', content, flags=re.DOTALL)
    
    # Then find and replace the initialEdges definition
    pattern_edges = r'(const initialEdges = )\[([^\[]*?)(?=\n\s*const|$);'
    content = re.sub(pattern_edges, f'const initialEdges = {edges_js};', content, flags=re.DOTALL)
